export async function getPositions() {
    const url = 'http://192.168.15.121:8000/positions/';
    try {
...
export async function getPosition(positionId) {
    let url = 'http://192.168.15.121:8000/positions/';
    url += positionId
    try {
        const response = await fetch(url);
...
export async function sendMessage(name, company, email, message) {
    ...
    const url = 'http://192.168.15.121:8000/contacts/';
...
export const createApplication = async (
    ...
    try {
        ...
        const response = await fetch('http://192.168.15.121:8000/applications/', {
            method: 'POST',
            body: formData,
        });
